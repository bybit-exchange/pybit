...
class AsyncP2PHTTP(AsyncClient):
    ...
    async def upload_chat_file(self, **kwargs) -> dict:
        """Not yet supported in async client; use sync pybit.unified_trading.HTTP.upload_chat_file.

        Additional information:
            https://bybit-exchange.github.io/docs/p2p/order/upload-chat-file
        """
        raise NotImplementedError(
            "upload_chat_file is not yet supported in the async client. "
            "Use pybit.unified_trading.HTTP.upload_chat_file (sync) instead."
        )
